...
# Run the application  
## for development, you can use --reload and a single worker, but for production, we use multiple workers for better performance.
CMD ["uvicorn", "src.main:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "1", "--reload"]
## for production, use the following command without --reload and with multiple workers for better performance.
