...
class TextChunker:
    ...
    ## explain chunk_text: This method takes the full text of a paper and splits it into overlapping chunks based on the specified chunk size and overlap size. 
    # It first checks if the text is empty or too small to be chunked. If the text is valid, it splits the text into words and iteratively creates chunks by selecting a range of words, 
    # reconstructing them into text, and calculating metadata such as character offsets and overlaps. The method returns a list of TextChunk objects, 
    # each containing the chunked text and associated metadata.
    ## how the overlapping works: The overlapping is achieved by moving the current position forward by (chunk_size - overlap_size) after creating each chunk.
    ## explain overlap_with_next and overlap_with_previous: The overlap_with_previous indicates how many words from the previous chunk are included in the current chunk, 
    # while overlap_with_next indicates how many words from the current chunk will be included in the next chunk.
    ...
    ## why we are adding header into every chunk: Adding the title and abstract as a header to each chunk provides context for the content of that chunk.
    ## This is especially useful for search and retrieval tasks, as it allows the model or search engine to understand the relevance of the chunk in relation to the overall paper.
    ...
    def _parse_sections(self, sections: Union[Dict[str, str], str, list]) -> Dict[str, str]:
        ## explain: This method takes the sections input, which can be a dictionary, a JSON string, or a list, and converts it into a standardized dictionary format. 
        # It handles different input types and ensures that the output is a dictionary where keys are section titles and values are section contents. 
        # If the input is a JSON string, it attempts to parse it into a dictionary or list and processes it accordingly.
        #  If parsing fails, it logs a warning and returns an empty dictionary.          
    ...
    ## why we are removing metadata sections: Metadata sections (like author info, affiliations, submission history) are not relevant for content-based chunking and indexing.
    ## They can introduce noise and reduce the quality of the chunks, so we filter them out to focus on meaningful content sections.
    ...
    def _create_combined_chunk(
        ...
        separator = "\n\n"
        combined_text = f"{header}{separator.join(combined_content)}"
        ...
        if total_words + len(header.split()) < 200 and existing_chunks:
            ...
            merged_text = f"{prev_chunk.text}{separator}{separator.join(combined_content)}"
