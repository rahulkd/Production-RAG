...
class RAGPromptBuilder:
    ...
    def create_bedrock_structured_prompt(self, query: str, chunks: List[Dict[str, Any]]) -> str:
        """Create a RAG prompt that asks for JSON matching the RAGResponse schema.

        The Bedrock Converse API has no equivalent of Ollama's ``format`` schema
        parameter, so the schema is inlined into the prompt instead. Output is
        still validated by :class:`ResponseParser`, which falls back to plain
        text if the model ignores the instruction.
        ...
        Returns:
            Formatted prompt string requesting a JSON object
        """
        schema = json.dumps(RAGResponse.model_json_schema(), indent=2)

        prompt = self.create_rag_prompt(query, chunks)
        prompt += (
            "\n\n### Output Format:\n"
            "Respond with a single JSON object and nothing else - no markdown fences, "
            "no commentary before or after. It must validate against this JSON schema:\n\n"
            f"{schema}\n"
        )
        return prompt
