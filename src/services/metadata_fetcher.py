    ...
    def _store_papers_to_db(
        ...
        for paper in papers:
            try:
                ...
                ## in upsert, already each paper is committed to db, then why we need to call db_session.commit() again after the loop?
                # The `upsert` method in the `PaperRepository` class does call `commit()` after adding or updating each paper, which means that each individual paper is persisted 
                # to the database immediately. However, calling `db_session.commit()` again after the loop can serve a few purposes:
                # 1. **Batching and Performance**: If the `upsert` method is modified to not call `commit()` after each operation (for example, if it only adds the paper to 
                # the session without committing), then calling `db_session.commit()` after the loop would allow all changes to be committed in a single transaction, which can 
                # improve performance when processing a large number of papers.
                # 2. **Error Handling**: If an error occurs during the processing of any paper, having a single commit at the end allows for easier rollback of all changes if 
                # something goes wrong, ensuring database integrity. If each `upsert` commits individually, it can be more difficult to manage partial failures.
                # 3. **Consistency**: Even if `upsert` commits each paper, calling `db_session.commit()` at the end can ensure that all changes are finalized and that any pending 
                # transactions are completed, especially if there are other operations being performed on the session outside of the `upsert` method.
                # In the current implementation, since `upsert` commits each paper, the final `db_session.commit()` may not be strictly necessary, but it can serve as a safeguard 
                # to ensure that all changes are properly committed and to handle any additional operations that may have been added to the session during the loop.
                if stored_paper:
                    stored_count += 1
