## Fetch the latest papers from arXiv, process them, store in PostgreSQL, and index to OpenSearch.
## no date filter to get the most recent papers, sorted by submission date (descending)
...
# The `get_cached_services` function is responsible for initializing and caching the core services used in the paper ingestion pipeline, 
# such as the arXiv client, PDF parser, database connection, and metadata fetcher. By using the `@lru_cache` decorator, 
# this function ensures that the services are only initialized once and then reused across multiple calls, 
# improving performance and resource utilization. This is particularly important in an Airflow DAG context where tasks may be executed 
# multiple times, and we want to avoid redundant initialization of services that can be expensive to set up.
@lru_cache(maxsize=1)
...
async def run_paper_ingestion_pipeline(
    ...
    Fetches the latest papers (sorted by submission date, descending) with no
    date filter, so arXiv returns the most recent `max_results` papers.
    ...
    """
    _arxiv_client, _pdf_parser, database, metadata_fetcher,opensearch_client = get_cached_services()
...
def fetch_daily_papers(**context):
    """
    Fetch the latest CS.AI papers.

    This function:
    1. Fetches the most recent papers using the MetadataFetcher (no date filter)
    2. Returns processing statistics
    """
    logger.info("Starting daily arXiv paper fetch")

    try:
        logger.info("Fetching the latest 10 papers (sorted by submission date)")

        ## what does max_results do in this context?
        # The `max_results` parameter limits the number of papers fetched from arXiv.
        # With no date filter, arXiv returns the most recent `max_results` papers
        # sorted by submission date (descending), so this fetches the latest 10.
...
def index_papers_to_opensearch(**context):
    ...
    try:
        ...
        with database.get_session() as session:
            ...
            # This SQL query retrieves all papers from the "papers" table that were created on the current date (i.e., the date of the DAG execution). 
            # It orders the results by the creation timestamp in descending order, and limits the number of results to the number of papers that were stored during 
            # the fetch task (or 100 if that information is not available).
            query = f"""
                SELECT * FROM papers 
